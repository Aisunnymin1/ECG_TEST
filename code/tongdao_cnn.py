...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.raw1 = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=(15, 1), stride=1, padding=(1, 0), bias=False),
        ...
        self.raw2 = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=(13, 1), stride=1, padding=(1, 0), bias=False),
            nn.BatchNorm2d(num_features=32),
            nn.Dropout(0.7),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=(3, 1))
        ...
        self.raw3 = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=(19, 1), stride=1, padding=(1, 0), bias=False),
            nn.BatchNorm2d(num_features=32),
            nn.Dropout(0.8),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=(3, 1))
        ...
        #     nn.Conv2d(128, 256, (5, 1), 1, 0),  # 直接简写了，参数的顺序同上
        #     nn.ReLU(),
        #     nn.MaxPool2d((3, 1))
        # )
        self.network = nn.Sequential(
            nn.Linear(3200 * 1, 1000),  # 卷积层后加一个普通的神经网络
            nn.Dropout(0.7),
