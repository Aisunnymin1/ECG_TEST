...
class ResNet34(nn.Module):
    ...
    def __init__(self):
        ...
        self.raw1 = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=(3, 1), stride=1, padding=(1, 0), bias=False),
        ...
        self.raw2 = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=(5, 1), stride=1, padding=(1, 0), bias=False),
            nn.BatchNorm2d(num_features=32),
            nn.Dropout(0.8),
            nn.ReLU(inplace=False),
            nn.MaxPool2d(kernel_size=(3, 1))
        ...
        self.raw3 = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=(7, 1), stride=1, padding=(1, 0), bias=False),
            nn.BatchNorm2d(num_features=32),
            nn.Dropout(0.8),
            nn.ReLU(inplace=False),
            nn.MaxPool2d(kernel_size=(3, 1))
        ...
        self.pre = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(13, 1), stride=1, padding=0, bias=False),
            nn.BatchNorm2d(num_features=32),
            nn.Dropout(0.8),
            nn.ReLU(inplace=False),
        ...
        self.network = nn.Sequential(
            nn.Linear(14208 * 1, 1000),  # 卷积层后加一个普通的神经网络
            nn.Dropout(0.7),
    ...
    def forward(self, x):
        # x = t.cat((self.raw1(x),self.raw2(x),self.raw3,self.raw4),dim=2)
        x1 = self.raw1(x)
        x2 = self.raw2(x)
        x3 = self.raw3(x)
        x4 = self.raw4(x)
        x = t.cat((x1, x2, x3, x4), dim=2)
        x = self.maxpool(x)
        x = self.pre(x)
        x = self.layer1(x)
        x = self.layer2(x)
...
PARM.datanum = 2000  # 提取数据量
PARM.len = 4000  # 数据截断长度
...
if PARM.ifGPU:
    ...
    test_y = test_y.cuda()
## 优化算法
optimizer = t.optim.Adam(cnn.parameters()
                             , lr=PARM.lr)  # 使用adam算法来进行优化，lr是学习率，学习率越高，学习速度越快，越低，精度可能会越高
## 损失函数
